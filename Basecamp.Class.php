...
class Basecamp {
	...
	function setOAuthAuthentication($client_id,$client_secret,$oauth_url,$session_key='oAuth') {
		...
		if (isset($_SESSION['oautha'])) {
			$this->access_token 	= $_SESSION['oautha'];
			$this->authenticated 	= true;
		}
	}
	function setOAuthAuthentication1($client_id,$client_secret,$oauth_url,$session_key='oAuth') {	
	...
	function getsummarymessages($id){
		return $this->_makeAuthenticatedRequest($this->getAccountURL("projects/$id/posts/archive.xml"));
        }
	//- mark Attachments
